...
if($_SESSION['mode'] == "prop"){
	...
	if(isset($_FILES['new-img'])){
		move_uploaded_file($_FILES['new-img']['tmp_name'][0], $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0]);
		$prp->chProprietaryImg($_SESSION['user'], $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0]);
		$_SESSION['user-icon'] = $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0];
	}
}
else{
	...
	if(isset($_FILES['new-img']) && strlen($_FILES['new-img']['name'][0]) > 0){
		move_uploaded_file($_FILES['new-img']['tmp_name'][0], $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0]);
		$usr->chImage($_SESSION['user'], $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0]);
		$_SESSION['user-icon'] = $_SERVER['DOCUMENT_ROOT'] . "/lpgp-server/u.images/" . $_FILES['new-img']['name'][0];
	}
}
sendUserLogged();
